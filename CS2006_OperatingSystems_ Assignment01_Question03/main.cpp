...
wstring DataTransformer::decryptData(const wstring &data) {
    ...
    for (wchar_t &c: decryptedData) {
        if (iswalpha(c)) {
            ...
            if (c == L'A') c = L'Z';   // wrapping around for uppercase 'A'
            else if (c == L'a') c = L'z';  // wrapping around for lowercase 'a'
            else c = c - 1;  // shifting character back by one
        } else if (iswdigit(c)) {
            // Handling numeric characters
            if (c == L'0') c = L'9';   // wrapping around for '0'
            else c = c - 1;  // shifting digit back by one
        } else if (iswspace(c)) {
            // keeping spaces unchanged
            continue;
        } else if (iswprint(c)) {
            // Handling other printable chars
            c = c - 1;  // shifting character back by one
            if (!iswprint(c)) {
...
wstring DataTransformer::redactData(const wstring &data) {
    ...
    wstring redactedData = data;

    // Defining regular expressions for sensitive information patterns

    // Email address pattern (plus point it also supports Unicode characters)
    wregex emailRegex(LR"(([\w.%+-]+)@([\w.-]+)\.([A-Za-z]{2,}))");

    // Credit card number pattern (supports kinda every formats)
    wregex ccRegex(LR"(\b(?:\d[ -]*?){13,16}\b)");

    // Adding few extra patterns to the code
    ...
    // Replacing credit card numbers with "[REDACTED]"
    redactedData = regex_replace(redactedData, ccRegex, L"[REDACTED]");

    // Replacing SSNs with "[REDACTED]"
